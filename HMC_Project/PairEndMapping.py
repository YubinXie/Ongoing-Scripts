...
for File in FileName: 
	line2=re.search('(?P<h1>.+?)_R(?P<h2>.+?).fastq',File)
	if(line2):
...
for sample in name1list:
	...
	input1name=name1list[i]+"_R1*_pairs_*.fastq.gz"
	input2name=name1list[i]+"_R2*_pairs_*.fastq.gz"
	outputname=name1list[i]+"_quality_paired.sam"
